...
export function FunctiesShowMore({ title, subtitle, items }: FunctiesShowMoreProps) {
  return (
    <div>
      <h3 className="text-4xl font-bold tracking-tight text-[#1d3014] sm:text-5xl md:text-6xl">
        {title}
      </h3>
      ...
      <div className="mt-8 space-y-4">
        {items.map((item) => (
          <details
            key={item.title}
            className="group border border-gray-200 dark:border-neutral-700 rounded-xl p-6 hover:border-[#1d3014]/50 transition-colors bg-white dark:bg-neutral-900"
          >
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-neutral-700">
              ...
              <Link
                href={item.demoHref}
                className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-[#1d3014] hover:underline underline-offset-2"
              >
                Bekijk in demo
