...
export function TrialBox({ collapsed }: { collapsed: boolean }) {
    ...
    return (
      <div className="flex-shrink-0 px-2 pb-2">
        <Link
          ...
          className="flex items-center justify-center w-full h-9 rounded-lg bg-[#1d3014] hover:bg-[#1e4500] transition-colors"
        >
          <span className="text-[10px] font-bold text-[#c8e957]/70">{days}d</span>
        </Link>
      </div>
    )
  }
  ...
  return (
    <div className="flex-shrink-0 px-3 pb-3">
      <div className="bg-[#1d3014] rounded-xl p-4 relative overflow-hidden">
        <GeometricShapes
          variant="trapezoid"
        ...
        <div className="relative z-10">
          ...
          <div className="h-1 rounded-full bg-white/15 mb-3 overflow-hidden">
            <div className="h-full rounded-full bg-[#c8e957]" style={{ width: `${pct}%` }} />
          </div>
          <Link
            href="/dashboard/landlord/upgrade"
            className="block w-full text-center bg-[#c8e957] text-[#1d3014] hover:bg-[#c8e957]/90 rounded-xl text-sm font-semibold py-2 transition-colors"
          >
            Abonnement starten
          </Link>
        </div>
