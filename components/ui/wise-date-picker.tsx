...
export function WiseDatePicker({ value, onChange, placeholder = 'Kies datum', min, max, triggerClassName, className }: WiseDatePickerProps) {
      ...
      <PopoverPrimitive.Trigger asChild>
        <button
          ...
          className={triggerClassName ?? cn(
            ...
            'hover:border-[#163300] transition-colors',
            !displayValue && 'text-muted-foreground',
            className
          )}
        >
          <span className={cn(!displayValue && 'text-gray-300 dark:text-neutral-600')}>
            {displayValue || placeholder}
          </span>
          <CalendarDays className="h-4 w-4 text-gray-300 dark:text-neutral-600 shrink-0" />
        </button>
