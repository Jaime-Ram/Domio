...
export function AuthLoadingScreen({ onAnimationComplete }: AuthLoadingScreenProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-[9999] min-h-screen flex items-center justify-center bg-[#c8e957] px-4 sm:px-6 overflow-hidden"
      style={{
        opacity: fadeOut ? 0 : 1,
...
function LogoReveal({ phase, revealPctValue, fadeZone, moveDurationMs }: LogoRevealProps) {
  ...
  return (
    <div
      className="flex items-baseline min-w-0 max-w-full text-[#1d3014] text-2xl sm:text-4xl md:text-5xl tracking-tight pb-[0.15em]"
      style={{
